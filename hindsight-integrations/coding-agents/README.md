...
| `pageTriggerType`       | `"reactive"`                         | when NEW knowledge pages refresh, i.e. what keeping them current costs — `"reactive"` after every consolidation that produced new material, `"cron"` on `pageTriggerCron` only, `"manual"` never on their own. Reactive is the most current and the most expensive: one synthesis per page per consolidation. Maps to the page's `trigger.refresh_after_consolidation` in the Hindsight API (`true` for reactive, `false` for manual) |
