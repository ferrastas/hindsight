...
| `pageTriggerType`       | `"reactive"`                         | when NEW knowledge pages refresh: `"reactive"` after every consolidation that produced new material, `"cron"` on `pageTriggerCron` only, `"manual"` never on their own. Reactive is the most current and the most expensive — one synthesis per page per consolidation                    |
| `pageTriggerCron`       | —                                    | schedule for `pageTriggerType: "cron"` — UTC, standard 5-field cron, e.g. `"0 3 * * *"`. A scheduled refresh is skipped when nothing changed                                                                                        |
| `pageTriggerMode`       | `"full"` (server default)            | how a refresh rebuilds a page: `"full"` re-synthesizes from scratch, `"delta"` edits the existing content surgically — markedly cheaper per refresh                                                                                 |
