...
| `HINDSIGHT_API_SEMANTIC_ANN_OVERSEARCH_FACTOR` | How much wider the semantic arm searches the vector index than the number of rows it asks for. An approximate (ANN) index explores a bounded candidate list and returns what it found, so that list — not the SQL `LIMIT` — decides both the quality of the ranking and how many rows the arm can return at all; the filters that thin the result (the similarity floor, tags, date ranges) are applied *after* the scan, so a query wanting N rows must explore more than N candidates. On pgvector this sizes `hnsw.ef_search` for the query, replacing the connection's standing value when the recall budget needs more, and is capped by that setting's accepted maximum of `1000`. Backends exposing no such knob (`vchord`, `pgvectorscale`, `pg_diskann`, `scann`) ignore it. Must be `>= 1.0`; `1.0` searches no wider than requested. | `2.0` |
