...
| `HINDSIGHT_API_SEMANTIC_OVERFETCH_FACTOR` | How far the semantic arm over-fetches past the rows it keeps, to absorb ANN approximation and the filters Postgres applies *after* the index scan (the similarity floor, tags, date ranges). The resolved request also sizes the vector backend's per-query candidate list for the same statement — on pgvector that is `hnsw.ef_search`, whose accepted maximum of `1000` therefore caps how wide the arm can usefully run, since an HNSW scan cannot return more rows than its candidate list holds. The arm never retrieves more than the reranker cap above will keep, so raising `HINDSIGHT_API_RERANKER_MAX_CANDIDATES` (or its per-budget override) is what lets a `budget=high` recall widen its dense arm. Must be `>= 1.0`; `1.0` disables the over-fetch. | `2.0` |
